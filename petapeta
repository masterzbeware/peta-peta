...
do
  ...
  function utility:Pop(object, shrink)
    ...
    local oldTransparency = object.ImageTransparency or 0
    ...
    coroutine.wrap(function()
      ...
      object.ImageTransparency = oldTransparency -- balik ke nilai awal, bukan 0
      clone:Destroy()
    end)()
...
uiScale.Parent = container.Main

local function updateUIScale()
	local viewport = cam and cam.ViewportSize or Vector2.new(1920, 1080)
	local scale = math.clamp(math.min(viewport.X / 1920, viewport.Y / 1080), 0.75, 1.15)
	uiScale.Scale = scale
end
...
workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
    ...
    -- Floating button (tombol melayang)
    local floating = utility:Create("ImageButton", {
      Name = "FloatingButton",
      Parent = container,
      BackgroundTransparency = 1,
      AnchorPoint = Vector2.new(0, 0.5),
      ...
      ImageColor3 = themes.Accent,
      ImageTransparency = 1, -- transparansi kotak
      ScaleType = Enum.ScaleType.Slice,
	...
	utility:Create("ImageLabel", {
		...
		ZIndex = 21,
		Image = "rbxassetid://89256853269073", -- icon menu
		ImageColor3 = themes.TextColor
	})
})

-- Biar tombolnya bisa digeser
utility:DraggingEnabled(floating)

-- Klik tombol = show/hide UI utama
floating.MouseButton1Click:Connect(function()
	utility:Pop(floating, 10)
	container.Main.Visible = not container.Main.Visible
end)
    ...
    local container = utility:Create("ScrollingFrame", {
      ...
      BackgroundTransparency = 1,
      BorderSizePixel = 0,
      Position = UDim2.new(0, 134, 0, 42),
      Size = UDim2.new(1, -142, 1, -46),
    
      CanvasSize = UDim2.new(0, 0, 0, 0),
      AutomaticCanvasSize = Enum.AutomaticSize.Y, -- <-- tambah ini
    
      ScrollingDirection = Enum.ScrollingDirection.Y,
      ScrollingEnabled = true,
      ScrollBarThickness = 8, -- <-- biar enak di mobile
...
local function updatePageCanvas()
	if not layout or not container.Parent then
		return
	end

	local contentY = layout.AbsoluteContentSize.Y + 10
	container.CanvasSize = UDim2.new(0, 0, 0, contentY)
	container.ScrollBarImageTransparency = (contentY > container.AbsoluteSize.Y) and 0 or 1
end

layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updatePageCanvas)
container:GetPropertyChangedSignal("AbsoluteSize"):Connect(updatePageCanvas)
...
task.defer(updatePageCanvas)
  ...
  function page:Resize(scroll)
    ...
    if layout then
      size = layout.AbsoluteContentSize.Y + padding
    else
  ...
  function section:Resize(smooth)
    ...
    if smooth then
      ...
      task.delay(0.06, function()
        if self.page and self.page.container and self.page.container.Parent then
          self.page:Resize()
        end
      end)
    else
